...
1. **Finish the gauntlet.** Local 0.1.2 checks and fail-fast disposable proofs pass; adversarial content/evidence review has no remaining CRITICAL/HIGH/MEDIUM finding. Use [PR #13](https://github.com/WithAutonomi/skills/pull/13) to inspect the current immutable revision, exact-revision reviews and ADR CI. Keep official Fable clean-context marked deferred until its dedicated route can run; do not substitute the failed fallback trace. Scenario A (free read) still needs a real host; B (paid write) and D (nodes) run only when a funded wallet and a host with room are available. Then seek approval to merge.
