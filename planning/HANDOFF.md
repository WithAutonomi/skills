...
**Latest action (24 September):** Jim authorised a local commit and official Claude review. Candidate `81f023e7d74c232dd04bd47a88bf82f09887cc8a` is committed. Review could not start: the shared review lock contains a separate Try Autonomi brief and atomic acquisition failed. The lock was left untouched; no Claude call, push or PR occurred. Resolve the lock through its owner or an authorised recovery before another attempt. This supersedes the pre-commit status in the preparation summary below; full evidence is in [the checkpoint](evidence/2026-Sep-24-npm-install.md).

The **one task-routed prototype (0.1.4)** merged through [PR #13](https://github.com/WithAutonomi/skills/pull/13) at `69ca49452e7555f607646bc9f023718bf43cd046`. The older operator-only skill is archived; do not edit it for current installation changes. On 24 September Jim approved **0.1.5 npm-first client installation**, being prepared locally on `autonomi-npm-install`. Main still carries 0.1.4 until this change is reviewed and merged. The new work keeps both direct installers in the bundled reference, updates npm lifecycle guidance and preserves all user/application state. Scope: [`PACKET-autonomi-npm-install.md`](packets/PACKET-autonomi-npm-install.md). Checks, review status and remaining gaps: [`npm-install evidence`](evidence/2026-Sep-24-npm-install.md). No commit, push or PR creation has been performed for this change. Prior review waivers applied to the September 6 reconciliation only, not this work. The prototype remains unproven across all live scenarios; no new Windows or node-operation support claim is made.
...
1. **Complete the npm-first review.** [PR #13](https://github.com/WithAutonomi/skills/pull/13) has merged; the 0.1.5 candidate is committed locally. Resolve the shared clean-context lock without disrupting its owner, then run the official review. Ask Jim before pushing or opening its PR. Current evidence is linked above.
