...
- **Repo:** `skills` — target `WithAutonomi/skills`, the org's first-party skills home (holds `skills/autonomi/` now; ready for siblings like `autonomi-developer`). **Done (2026-06-24):** renamed `JimCollinson/autonomi-skill` → `JimCollinson/skills` (Jim's action; GitHub auto-redirects old URLs) so the later move to the org is a one-hit ownership transfer with the name already correct. Transfer to `WithAutonomi` is a gated step.
