...
- **Bump the version with the skill.** Any change to a shipped file (`skills/autonomi/SKILL.md`, `VERSION`, `references/`) bumps `skills/autonomi/VERSION`, the frontmatter's `metadata.version`, and both Claude plugin versions together. Install channels use their own version or source-hash records to deliver updates.
