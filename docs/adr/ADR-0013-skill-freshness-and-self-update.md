...
- **Best-effort and bundle-backed.** Reachable and verified → the value informs **observation and reporting**, and reaches behaviour only through the apply policy below. Unreachable, unverifiable, expired, or conflicting → **fall back to the bundled, team-confirmed values**. Sustaining on bundled values is a **safety-line** guarantee, not a guarantee of operational optimality; **prolonged** unreachability is itself an escalation trigger.
